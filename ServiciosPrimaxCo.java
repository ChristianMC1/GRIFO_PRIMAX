...
public class ServiciosPrimaxCo {
    ...
    public static void generarBoleta() {
        ...
        try {
            ...
            exportarBoleta(nombreCliente, carrito, carritoPrecios, subtotal, igv, total, vuelto, pago);
            carrito.clear();
            carritoPrecios.clear();
            System.exit(0);

        } catch (NumberFormatException e) {
            System.out.println("⚠️ Monto inválido. Intenta de nuevo.");
        }
    }

    public static void exportarBoleta(String cliente, ArrayList<String> productos, ArrayList<Double> precios,
                                      double subtotal, double igv, double total, double vuelto, double pago) {
        ...
        try (FileWriter archivo = new FileWriter(archivoNombre)) {
            ...
            for (int i = 0; i < productos.size(); i++) {
                archivo.write("- " + productos.get(i) + ": $" + String.format("%.2f", precios.get(i)) + "\n");
            }
            archivo.write("\n");
            archivo.write(String.format("Subtotal: $%.2f\n", subtotal));
            archivo.write(String.format("IGV (18%%): $%.2f\n", igv));
            archivo.write(String.format("Total: $%.2f\n", total));
            archivo.write(String.format("Pago realizado: $%.2f\n", pago));
            archivo.write(String.format("Vuelto: $%.2f\n", vuelto));
            archivo.write("¡Gracias por su compra!\n");

            System.out.println("Boleta exportada automáticamente como '" + archivoNombre + "'.");
        ...
        while (!salir) {
            ...
            switch (ven) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    // Esta opción no necesita acción ya que generarBoleta() ya exporta
